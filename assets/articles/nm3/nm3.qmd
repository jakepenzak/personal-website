---
title: 'Optimization, Newton''s Method, & Profit Maximization: Part 3 - Applied Profit Maximization'
jupyter: python3
toc: true
execute:
  cache: true
...
parallax_split_cover(
    ...
    kicker="Econometrics • Optimization",
    title='Optimization, Newton''s Method, & Profit Maximization: Part 3 - Applied Profit Maximization',
    subtitle="Learn how to apply optimization & econometric techniques to solve an applied profit maximization problem.",
    chips=("Profit Maximization", "Demand Modeling", "Constrained Opt."),
    takeaways=(
        "Translate a business objective into a constrained optimization problem.",
        "Use an econometric demand model to connect ad spend to revenue.",
        "Solve budget + minimum-spend constraints with Lagrangians and log barriers.",
    ),
    equation_latex=r"\max_{\delta,\tau}\;\pi(\delta,\tau)\ \ \text{s.t.}\ \delta+\tau=B",
)
```
...
```{python}
#| code-fold: true
#| code-summary: Functions Defined in Part 1 & Part 2
...
```{python}
#| code-fold: true
#| code-summary: Parameter Definition
...
```{python}
#| code-fold: true
#| code-summary: AR Data Simulation
...
```
![](data/quantity_demanded_plot.webp)
...
$$
\hat{q}(\delta,\tau,\cdot) = 10071.28*\log(δ) + 6219.99*\log(τ) + 21336.81
$$
...
$$
\begin{aligned}
\pi(\delta,\tau,\cdot) &= -δ - τ - (-1.01*\log(δ) \\
&- 0.62*\log(τ) + 137.87)*(10071.28*\log(δ) \\
&+ 6219.99*\log(τ) + 21336.81) \\
&+ 1812830.32*\log(δ) + 1119598.67*\log(τ) \\
&+ 3840626.12
\end{aligned}
$$
...
```{=html}
<center><iframe src="data/profit_function_3d.html" width="750" height="600" frameborder="0"></iframe></center>
```
...
# Define the grid
δ_grid = np.linspace(0.01, 100000, 1000)
τ_grid = np.linspace(0.01, 100000, 1000)
X, Y = np.meshgrid(δ_grid, τ_grid)
Z = profit_fn(X, Y)
...
plt.fill_betweenx(
    τ_grid, 0, 10_000, color="gray", alpha=0.5
)  # Shade region where x > 0
plt.fill_between(
    δ_grid, 0, 20_000, color="gray", alpha=0.5
...
optimums = constrained_newtons_method(objective, symbols, x0, rho_steps=1000)
```
