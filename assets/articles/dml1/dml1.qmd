---
title: 'Double Machine Learning, Simplified: Part 1 - Basic Causal Inference Applications'
jupyter: python3
toc: true
format:
  live-html:
...
parallax_split_cover(
    ...
    kicker="Causal ML • Double/Debiased ML",
    title='Double Machine Learning, Simplified: Part 1 - Basic Causal Inference Applications',
    subtitle="Learn how to utilize DML in causal inference tasks.",
    chips=("Partialling Out", "Cross-Fitting", "Neyman Orthogonality"),
    takeaways=(
        "DML estimates causal effects by residualizing y and T with flexible ML models.",
        "Cross-fitting reduces overfitting bias and stabilizes inference.",
        "You can recover the true ATE even under highly non-linear confounding.",
    ),
    equation_latex=r"y-\mathcal{M}_y(X)=\beta\,(T-\mathcal{M}_T(X))+\varepsilon",
)
```
...
df = pd.DataFrame(
    np.array([sales,time_on_website,age,num_social_media_profiles,yr_membership,Z,collider]).T,
    columns=["sales","time_on_website","age","num_social_media_profiles","yr_membership","Z","collider"]
)
```
...
HTML(f"<center>{regtable.render_html()}</center><br>")
```
...
Furthermore, suppose the following DGP:
...
df_rct = pd.DataFrame(
    np.array([sales_rct,advertisement_exposure,age,num_social_media_profiles,yr_membership,Z]).T,
