---
title: 'Optimization, Newton''s Method, & Profit Maximization: Part 2 - Constrained Optimization Theory'
jupyter: python3
toc: true
format:
  live-html:
...
parallax_split_cover(
    ...
    kicker="Optimization • Constraints",
    title='Optimization, Newton''s Method, & Profit Maximization: Part 2 - Constrained Optimization Theory',
    subtitle="Learn how to solve constrained optimization problems.",
    chips=("Lagrangian", "Barrier Methods", "Interior-Point"),
    takeaways=(
        "Equality constraints can be handled via Lagrange multipliers in a single augmented objective.",
        "Inequality constraints can be solved with log barriers that keep iterates feasible.",
        "Combining Lagrangians + barriers yields a practical template for mixed constraints.",
    ),
    equation_latex=r"\mathcal{O}(x)=f(x)+\lambda\,h(x)-\rho\,\log(c(x))",
)
```
...
```{python}
#| code-fold: true
#| code-summary: Functions Defined in Part 1
...
newtons_method(objective, Gamma, Gamma0)
...
```{=html}
<center><iframe src="data/eqc_rosenbrocks_viz_3d.html" width="750" height="600" frameborder="0"></iframe></center>
```
...
newtons_method(lagrangian, Gamma, Gamma0)
```
...
```
```{=html}
<center><iframe src="data/ineqc_rosenbrocks_viz_3d.html" width="750" height="600" frameborder="0"></iframe></center>
```
